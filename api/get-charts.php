...
try {
    ...
    $charts = $db->fetchAll(
        "SELECT * FROM music_charts WHERE chart_date = :date AND country_code = :country ORDER BY rank ASC LIMIT :limit",
        ['date' => $date, 'country' => $countryCode, 'limit' => $limit]
    );
    
    // If no charts for country, get global
    if (empty($charts)) {
        $charts = $db->fetchAll(
            "SELECT * FROM music_charts WHERE chart_date = :date ORDER BY rank ASC LIMIT :limit",
    ...
    error_log("API Error (get-charts): " . $e->getMessage());
    errorResponse('An error occurred while fetching charts', 500);
}
